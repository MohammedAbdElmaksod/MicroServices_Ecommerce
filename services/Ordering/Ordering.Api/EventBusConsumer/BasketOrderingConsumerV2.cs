...
public class BasketOrderingConsumerV2 : IConsumer<BasketCheckoutEventV2>
{
    ...
    {
        using var scope = _logger.BeginScope($"Checkout Order Event Consume here with correlationId V2 = {context.Message.CorrelationId}");
        var cmd = _mapper.Map<CheckoutOrderCommandV2>(context.Message);
        var result = await _mediator.Send(cmd);
        _logger.LogInformation($"Consume Order Event completed V2");
    }
}
