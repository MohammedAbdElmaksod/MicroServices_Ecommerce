...
public class BasketOrderingConsumer : IConsumer<BasketCheckoutEvent>
{
    ...
    {
        using var scope = _logger.BeginScope($"Checkout Order Event Consume here with correlationId = {context.Message.CorrelationId}");
        var cmd = _mapper.Map<CheckoutOrderCommand>(context.Message);
        var result = await _mediator.Send(cmd);
        _logger.LogInformation($"Consume Order Event completed");
    }
}
